...
import { RotateCcw, Check, X, Star, Clock, ArrowRight, Target, Pencil, RefreshCw } from "lucide-react";
...
function ResultScreen({ result, onNext, onRetry }: { result: MiniGameResult; onNext: () => void; onRetry: () => void }) {
  const getMessage = (stars: number) => {
    if (stars === 3) return { text: "Luar Biasa!", sub: "Sempurna tanpa kesalahan!", icon: <Star size={24} color="#FFD93D" fill="#FFD93D" /> };
    if (stars === 2) return { text: "Bagus Banget!", sub: "Hampir sempurna, terus semangat!", icon: <Star size={24} color="#FF9600" fill="#FF9600" /> };
    return { text: "Yay, Kamu Berani!", sub: "Sudah mencoba dengan baik!", icon: <Star size={24} color="#1CB0F6" fill="#1CB0F6" /> };
  };
